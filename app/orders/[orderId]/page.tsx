  ...
  return (
    <main className="min-h-screen bg-[#f3f1ed] px-4 py-12 text-black">
      <div className="mx-auto max-w-2xl">
        <Link href="/orders" className="text-sm font-semibold text-black/55 hover:text-black">← Your orders</Link>
          ...
          <div className="mt-7 border-t border-black/10 pt-6">
            {order.items.map((item) => (
              <div key={item.id} className="flex items-center justify-between gap-4 py-2 text-sm">
                <span>{item.quantity}× {item.product.replaceAll("_", " ")} · {item.fabric.replaceAll("_", " ")}</span>
                <strong>{order.currency} {(item.totalCents / 100).toFixed(2)}</strong>
              </div>
            ))}
            <div className="mt-3 flex items-center justify-between border-t border-black/10 pt-4 text-lg">
              <strong>Total</strong><strong>{order.currency} {(order.totalCents / 100).toFixed(2)}</strong>
            </div>
          </div>
        </section>
      </div>
