...
async function statusResponse(orderId: string) {
  ...
  const order = await prisma.order.findUnique({
    where: { id: orderId },
    select: { id: true, userId: true, orderNumber: true, paymentStatus: true, status: true, totalCents: true, currency: true },
  });
...
export async function POST(req: Request) {
  ...
  if (typeof body?.orderId !== "string") return NextResponse.json({ error: "orderId is required." }, { status: 400 });
...
export async function GET(req: Request) {
  ...
  const order = await prisma.order.findFirst({ where: { paymobOrderId: remoteOrderId }, select: { id: true, paymentStatus: true } });
  if (!order) return NextResponse.redirect(new URL("/orders", url.origin));
  const destination = order.paymentStatus === PaymentStatus.PAID
